...
def parse_command_line():
    ...
    advanced= parser.add_argument_group(
        ...
        description="See the PySeg documentation for a more detailed description of these parameters.")
    ...
    advanced.add_argument('--v_mode', default=v_mode, type=str, help="If 'high' then vertices with highest property values are preserved.")
    advanced.add_argument('--e_mode', default=e_mode, type=str, help="If 'high' then edges with highest property values are preserved.")
    advanced.add_argument('--prop_topo', default=prop_topo, type=str, help='GraphMCF property.')
    
    return parser.parse_args()

### Parallel worker
def pr_worker(pr_id, ids, q_pkls=None):
    pkls_dic = dict()
    ...
    for row in ids:
        ...
        print(f'{time.strftime( "%Y-%m-%d %H:%M:%S", time.localtime() )} Process[{str(pr_id)}] Sub-volume to process found: {input_tomo}', file=sys.stdout, flush=True)
        print(f'{time.strftime( "%Y-%m-%d %H:%M:%S", time.localtime() )} Process[{str(pr_id)}] Computing paths for: {input_tomo}', file=sys.stdout, flush=True)
        ...
        ###print('\tP[' + str(pr_id) + '] Loading input data: ' + stem_tomo, file=sys.stdout, flush=True)
        print(f'{time.strftime( "%Y-%m-%d %H:%M:%S", time.localtime() )} Process[{str(pr_id)}] Loading input data: {stem_tomo}', file=sys.stdout, flush=True)
        tomo = ps.disperse_io.load_tomo(input_tomo).astype(np.float32)
        segh = ps.disperse_io.load_tomo(input_seg)

        ###print('{time.strftime( "%Y-%m-%d %H:%M:%S", time.localtime() )} Process[' + str(pr_id) + '] Computing distance, mask and segmentation tomograms...', file=sys.stdout, flush=True)
        print(f'{time.strftime( "%Y-%m-%d %H:%M:%S", time.localtime() )} Process[{str(pr_id)}] Computing distance, mask and segmentation tomograms...', file=sys.stdout, flush=True)
        ...
        print(f'{time.strftime( "%Y-%m-%d %H:%M:%S", time.localtime() )} Process[{str(pr_id)}] Smoothing input tomogram (s={str(s_sig)})...', file=sys.stdout, flush=True)
        ...
        print(f'{time.strftime( "%Y-%m-%d %H:%M:%S", time.localtime() )} Process[{str(pr_id)}] Initializing DisPerSe...', file=sys.stdout, flush=True)
        ...
        #     print e.get_message()
            ...
            print('Jol!!!', file=sys.stdout, flush=True)
        ...
        print(f'{time.strftime( "%Y-%m-%d %H:%M:%S", time.localtime() )} Process[{str(pr_id)}] Persistence cut thereshold set to: {str(rcut)} grey level', file=sys.stdout, flush=True)
        disperse.set_cut(rcut)
        disperse.set_mask(input_msk)
        disperse.set_smooth(smooth)
        print(f'{time.strftime( "%Y-%m-%d %H:%M:%S", time.localtime() )} Process[{str(pr_id)}] Running DisPerSe...', file=sys.stdout, flush=True)
        return_code= disperse.mse(no_cut=False, inv=False)
        
        skel = disperse.get_skel()
        manifolds, return_code = disperse.get_manifolds(no_cut=False, inv=False)
        ##print(f"243 manifolds : {type(manifolds)}", file=sys.stdout, flush=True)
        ##print(f"244 return_code : {return_code}", file=sys.stdout, flush=True)

        # Build the GraphMCF for the membrane
        print(f'{time.strftime( "%Y-%m-%d %H:%M:%S", time.localtime() )} Process[{str(pr_id)}] Building MCF graph...', file=sys.stdout, flush=True)
        ...
        print(f'{time.strftime( "%Y-%m-%d %H:%M:%S", time.localtime() )} Process[{str(pr_id)}] Filtering nodes close to mask border...', file=sys.stdout, flush=True)
        ...
        print(f'{time.strftime( "%Y-%m-%d %H:%M:%S", time.localtime() )} Process[{str(pr_id)}] Building geometry...', file=sys.stdout, flush=True)
        graph.build_vertex_geometry()

        if do_clahe:
            print(f'{time.strftime( "%Y-%m-%d %H:%M:%S", time.localtime() )} Process[{str(pr_id)}] CLAHE on filed_value_inv property...', file=sys.stdout, flush=True)
        ...
        print(f'{time.strftime( "%Y-%m-%d %H:%M:%S", time.localtime() )} Process[{str(pr_id)}] Computing vertices and edges properties...', file=sys.stdout, flush=True)
        ...
        print(f'{time.strftime( "%Y-%m-%d %H:%M:%S", time.localtime() )} Process[{str(pr_id)}] Applying general thresholds...', file=sys.stdout, flush=True)
        if ang_rot is not None:
            print(f'{time.strftime( "%Y-%m-%d %H:%M:%S", time.localtime() )} Process[{str(pr_id)}] Deleting edges in MW area...', file=sys.stdout, flush=True)
            graph.filter_mw_edges(ang_rot, ang_tilt)

        print(f'{time.strftime( "%Y-%m-%d %H:%M:%S", time.localtime() )} Process[{str(pr_id)}] Computing graph global statistics (before simplification)...', file=sys.stdout, flush=True)
        nvv, nev, nepv = graph.compute_global_stat(mask=mask_den)
        print(f'\t{time.strftime( "%Y-%m-%d %H:%M:%S", time.localtime() )} Process[' + str(pr_id) + '] Vertex density: ' + str(round(nvv, 5)) + ' nm^3', file=sys.stdout, flush=True)
        print(f'\t{time.strftime( "%Y-%m-%d %H:%M:%S", time.localtime() )} Process[' + str(pr_id) + '] Edge density: ' + str(round(nev, 5)) + ' nm^3', file=sys.stdout, flush=True)
        print(f'\t{time.strftime( "%Y-%m-%d %H:%M:%S", time.localtime() )} Process[' + str(pr_id) + '] Edge/Vertex ratio: ' + str(round(nepv, 5)), file=sys.stdout, flush=True)

        print(f'{time.strftime( "%Y-%m-%d %H:%M:%S", time.localtime() )} Process[{str(pr_id)}] Graph density simplification for vertices...', file=sys.stdout, flush=True)
        ...
        try:
            ...
            print(f'{time.strftime( "%Y-%m-%d %H:%M:%S", time.localtime() )} Process[{str(pr_id)}] WARNING: graph density simplification failed:', file=sys.stdout, flush=True)
            print('\t-' + e.get_message())

        print(f'{time.strftime( "%Y-%m-%d %H:%M:%S", time.localtime() )} Process[{str(pr_id)}] Graph density simplification for edges in the membrane...', file=sys.stdout, flush=True)
        mask_mb = (seg == 1) * (~mask)
        ...
        print(f'{time.strftime( "%Y-%m-%d %H:%M:%S", time.localtime() )} Process[{str(pr_id)}] Computing graph global statistics (after simplification)...', file=sys.stdout, flush=True)
        nvv, _, _ = graph.compute_global_stat(mask=mask_den)
        _, nev_mb, nepv_mb = graph.compute_global_stat(mask=mask_mb)
        print(f'\t\tProcess[' + str(pr_id) + '] Vertex density (membrane): ' + str(round(nvv, 5)) + ' nm^3', file=sys.stdout, flush=True)
        print(f'\t\tProcess[' + str(pr_id) + '] Edge density (membrane):' + str(round(nev_mb, 5)) + ' nm^3', file=sys.stdout, flush=True)
        print(f'\t\tProcess[' + str(pr_id) + '] Edge/Vertex ratio (membrane): ' + str(round(nepv_mb, 5)), file=sys.stdout, flush=True)

        print('\nComputing graph properties (2)...', file=sys.stdout, flush=True)
        ...
        print('\n{time.strftime( "%Y-%m-%d %H:%M:%S", time.localtime() )} Saving intermediate graphs...', file=sys.stdout, flush=True)
...
print('Extracting GraphMCF and NetFilament objects from tomograms', file=sys.stdout, flush=True)
...
print('\tNumber of parallel processes: ' + str(npr), file=sys.stdout, flush=True)
print('\tDisPerSe persistence threshold (csig): ' + str(csig), file=sys.stdout, flush=True)
if ang_rot is not None:
    print('Missing wedge edge compensation (rot, tilt): (' + str(ang_rot) + ', ' + str(ang_tilt) + ')', file=sys.stdout, flush=True)
print('\tSigma for Gaussian pre-processing: ' + str(s_sig), file=sys.stdout, flush=True)
print('\tSigma for contrast enhancement: ' + str(nstd), file=sys.stdout, flush=True)
print('\tSkeleton smoothing factor: ' + str(smooth), file=sys.stdout, flush=True)
print('\tData resolution: ' + str(res) + ' nm/pixel', file=sys.stdout, flush=True)
print('\tMask offset: ' + str(mb_dst_off) + ' nm', file=sys.stdout, flush=True)
print('\tOutput directory: ' + output_dir, file=sys.stdout, flush=True)

print('\nGraph density thresholds:', file=sys.stdout, flush=True)
if v_prop is None:
    print('\tTarget vertex density (membrane) ' + str(v_den) + ' vertex/nm^3 for topological simplification', file=sys.stdout, flush=True)
else:
    print('\tTarget vertex density (membrane) ' +
        str(v_den) + ' vertex/nm^3 for property ' + v_prop + ' with mode ' + v_mode, file=sys.stdout, flush=True)
print('\tTarget edge/vertex ratio (non membrane) ' + str(ve_ratio) + ' for property ' + e_prop + ' with mode ' + e_mode, file=sys.stdout, flush=True)
if do_clahe:
    print('\t-Computing CLAHE.', file=sys.stdout, flush=True)
print('', file=sys.stdout, flush=True)

print('Paring input star file...', file=sys.stdout, flush=True)
star = ps.sub.Star()
star.load(in_star)
...
if not shutil.which(ps.globals.variables.DPS_MSE_CMD):
    print("ERROR!! DisPerSE executable 'mse' not found! Exiting...", file=sys.stdout, flush=True)
    exit()

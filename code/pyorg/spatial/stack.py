...
from .sparse import PlotUni, UniStat
...
def purge_stack(tomo, mask=None, axis=1, ratio=1):
    ...
    if (not isinstance(tomo, np.ndarray)) or (len(tomo.shape) != 3):
        ...
        raise pexceptions.PySegInputError(expr='purge_stack', msg=error_msg)
    if mask is not None:
...
class TomoUni(object):
    ...
    def __init__(self, tomo, mask=None, res=1., spacing=None, axis=2, name=None, area=(4, 20), ec=.9, pre=False):
        ...
        if axis == 0:
            ...
            raise pexceptions.PySegInputError(expr='__init__ (TomoUni)', msg=error_msg)
    ...
    def set_mask(self, mask):
        if (not isinstance(mask, np.ndarray)) or (len(mask.shape) != 3):
            ...
            raise pexceptions.PySegInputError(expr='set_mask (TomoUni)', msg=error_msg)
    ...
    def generate_tomo_stack(self):
        ...
        if (len(self.__stack) == 0) or (self.__m <= 0) or (self.__n <=0):
            ...
            raise pexceptions.PySegInputError(expr='generate_tomo_stack (TomoUni)', msg=error_msg)
    ...
    def homo_stack(self, freq, mode_3d=True):
        ...
        if mode_3d:
            homo = globals.tomo_butter_low(tomo, freq, n=5)
        else:
            ...
            for z in range(tomo.shape[2]):
                ...
                hold_tomo[:, :, 0] = tomo[:, :, z]
                flt = globals.tomo_butter_low(hold_tomo, freq, n=5)
                # disperse_io.save_numpy(flt, '/home/martinez/workspace/disperse/data/marion/cluster/out/flt.mrc')
                homo[:,:, z] = flt[:, :, 0]
    ...
    def __find_coords(self, pre=False):
        ...
        if len(self.__stack) == 0:
            ...
            raise pexceptions.PySegInputWarning(expr='__find_coords (TomoUni)', msg=error_msg)
...
class StackPlotUni(PlotUni):
    ...
    def analyze_stack_L(self, block=True, out_file=None, bar=False, mode='3d'):
        ...
        if self._PlotUni__Ks is None:
            ...
            raise pexceptions.PySegInputError(expr='analyze_stack_J (StackPlotUni)', msg=error_msg)
        cmap = cm.jet

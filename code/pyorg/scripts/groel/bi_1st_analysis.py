...
ROOT_PATH = '/fs/pool/pool-ruben/antonio/groel'

# Input STAR file
in_star_ref = ROOT_PATH + '/ltomos/tomo12006_all/all_ltomos.star' # '/ves_40/ltomos_premb_mask/premb_mask_ltomos.star'
in_ref_short_key = '3'
in_star = ROOT_PATH + '/ltomos/tomo12006_all/all_ltomos.star' # '/pst/nrt/ltomos_clst_flt_high/k4_gather_clst_flt_high_ltomos.star'
...
# Output directory
out_dir = ROOT_PATH + '/ana/tomo12006'
out_stem = 'bi_1st_ref_3_50_150_sim_20_switch_over0.05' # ''uni_sph_4_60_5'
...
if in_wspace is None:
    ...
    for row in range(star.get_nrows()):
        ...
        try:
            if p_vtp is not None:
                ...
                part_vtp = star.get_element('_suSurfaceVtpSim', row)
        except pexceptions.PySegInputError as e:
    ...
    for lkey in lists_hash.values():
        ...
        for tkey in tomos_hash.keys():
            ...
            if p_switch:
                temp_model = ModelCSRV(ref_tomo.get_voi(), disperse_io.load_poly(part_vtp), check_inter=p_cinter)
                sim_tomos = gen_tlist(p_nsims, ref_tomo.get_num_particles(), temp_model, ref_tomo.get_voi(), part_vtp,
                                      mode_emb='center', npr=ana_npr_model, tmp_folder=tmp_sim_folder)
            else:
                temp_model = ModelCSRV(ltomo.get_voi(), disperse_io.load_poly(part_vtp), check_inter=p_cinter)
                sim_tomos = gen_tlist(p_nsims, ltomo.get_num_particles(), temp_model, ltomo.get_voi(), part_vtp,
                                      mode_emb='center', npr=ana_npr_model, tmp_folder=tmp_sim_folder)
            for sim_tomo in sim_tomos.get_tomo_list():
                if p_switch:
                    hold_arr_dsts = sim_tomo.compute_bi_1st_order_dsts(ltomo.get_particle_coords())
                else:

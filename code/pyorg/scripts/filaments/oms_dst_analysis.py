    ...
    for lkey, llist in zip(iter(set_lists_dic.keys()), iter(set_lists_dic.values())):
        ...
        for tkey, ltomo in zip(iter(llist_tomos_dic.keys()), iter(llist_tomos_dic.values())):
            try:
                ...
                tomos_exp_dsts[tkey], tomos_exp_vols[tkey], tomos_exp_occ[tkey] = dict.fromkeys(list(lists_dic.keys())), \
                                                                                  dict.fromkeys(list(lists_dic.keys())), \
                                                                                  dict.fromkeys(list(lists_dic.keys()))
                tomos_count += 1
...
n_lists, lists_color = len(list(lists_hash.keys())), dict()
...
for tkey, ltomo in zip(iter(tomos_exp_vols.keys()), iter(tomos_exp_vols.values())):
    for lkey, arr in zip(iter(tomos_exp_vols[tkey].keys()), iter(tomos_exp_vols[tkey].values())):
        if len(arr) <= 0:
            print('\t\t\t+WARNING: no valid simulations for tomogram and list: ' + tkey + ', ' + lkey)
            continue
        tkey_short = os.path.splitext(os.path.split(tkey)[1])[0]
...
exp_hist_dic = dict().fromkeys(list(lists_exp_dsts.keys()))
...
for i, lkey in enumerate(exp_hist_dic.keys()):
    lkey_short = os.path.splitext(os.path.split(lkey)[1])[0]
    if lkey_short == '0':
        lkey_short = 'WT'
    elif lkey_short == '1':
...
plt.ylabel('Cumulative Density')
plt.xlabel('Inter-Membrane Nearest Distance [nm]')
exp_cdf_dic = dict().fromkeys(list(lists_exp_dsts.keys()))
...
plt.figure()
plt.ticklabel_format(style='sci', axis='y', scilimits=(0, 0))
exp_vols_dic = dict().fromkeys(list(lists_exp_dsts.keys()))
for lkey in exp_cdf_dic.keys():
    exp_cdf_dic[lkey] = list()
for tkey, ltomo in zip(iter(tomos_exp_dsts.keys()), iter(tomos_exp_dsts.values())):
    tkey_short = os.path.splitext(os.path.split(tkey)[1])[0]
...
colors = list()
for lkey in lists_exp_vols.keys():
    lkey_short = os.path.splitext(os.path.split(lkey)[1])[0]
    if lkey_short == '0':
        lkey_short = 'WT'
    ...
    n_lists += 1
boxp = plt.boxplot(list(exp_vols_dic.values()), showfliers=False, notch=True, vert=True, patch_artist=True,
                   labels=list(exp_vols_dic.keys()), boxprops={'linewidth': 2}, medianprops={'linewidth': 2, 'color':'black'},
                   whiskerprops={'linewidth': 2})
...
for idx, lkey in zip(np.arange(1, 3*len(list(lists_exp_occ.keys()))), iter(lists_exp_occ.keys())):
...
if len(list(lists_exp_occ.values())) >= 2:
    print('\t\t-Statistical test for organelle occupancy for the first two list: ')
    print('\t\t\t\t-K-S 2 samples [100*(1-p)]: ' + str(100. * (1 - sp.stats.ks_2samp(list(lists_exp_occ.values())[0],
                                                                                 list(lists_exp_occ.values())[1])[1])) + '%')

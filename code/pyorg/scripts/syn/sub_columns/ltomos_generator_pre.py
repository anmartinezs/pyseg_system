...
# Input STAR file
in_star = ROOT_PATH + '/in/pre_org.star'

# Input STAR for with the sub-volumes segmentations
in_seg = '/fs/pool/pool-lucic2/antonio/workspace/psd_an/in/syn_seg_no_l14_gap_fits.star'

# Output directory
out_dir = ROOT_PATH + '/ltomos/ltomos_pre_org_pre_swapxy_ssup5_min5' # '/stat/ltomos/trans_run2_test_swapxy'
out_stem = 'pre_org' # 'pre'

pt_res = 0.684 # nm/vx - resolution

#### Advanced settings

# Segmentation pre-processing
sg_lbl = 2 # segmented label
sg_bc = False # True
sg_bm = 'center' # Embedding checking mode
sg_pj = True # Project particles to VOI's surfaces
sg_origins = (1, 1, 1) # If not None, subtomoavg shiftings are considered,
                    # then scale factor (one per each pattern in in_star) from picked particle to subtomograms
sg_swap_xy = True # Swap X and Y coordinates of the input particle STAR files
...
pt_min_parts = 5 # Tomograms with less particles are removed
...
for star_row in range(star.get_nrows()):
    ...
    for part_row in range(star_part.get_nrows()):
        ...
        try:
            ...
            print('WARNING: particle in row ' + str(part_row) + ' could not be inserted in tomogram ' + mic_str + \

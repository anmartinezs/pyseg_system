...
in_wspace = None # ROOT_PATH + '/pst/ampar_vs_nmdar/org/col_scol/col_0_aln_15_sim_5_nn1_1_nn2_1_nn3_1_test2/col_0_aln_15_sim_5_nn1_1_nn2_1_nn3_1_test_wspace.pkl'

# Output directory
out_dir = ROOT_PATH + '/pst/ampar_vs_nmdar/org/col_scol/pre_tether_pst_col_0_aln_15_sim_1_nn1_1_nn2_1_nn3_1_test'
out_stem = 'pre_tether_pst_col_0_aln_15_sim_1_nn1_1_nn2_1_nn3_1_test' # ''uni_sph_4_60_5'
...
print('\t\t-Column radius: ' + str(ana_col_rad) + ' nm / ' + str(ana_col_rad_v) + ' vx')
...
if hasattr(l_id3, '__len__'):
    hold_set_list = surf.SetListTomoParticles()
    for idx in l_id3:
        ltomos_pkl = star_3.get_element('_psPickleFile', idx)
        hold_set_list.add_list_tomos(unpickle_obj(ltomos_pkl), key_3 + '_' + str(idx))
    list_3 = hold_set_list.merge_to_one_list()
    out_mrg_dir = out_dir + '/list_merged_3'
    if not os.path.exists(out_mrg_dir):
        os.makedirs(out_mrg_dir)
    out_mrg = out_mrg_dir + '/list_merged_3.pkl'
    print('\t\tStoring the merged list in directory: ' + out_mrg)
    try:
        list_3.pickle(out_mrg)
    except pexceptions.PySegInputError as e:
        print('ERROR: list of tomograms container pickling failed because of "' + e.get_message() + '"')
        print('Terminated. (' + time.strftime("%c") + ')')
        sys.exit(-1)
    out_app = out_mrg_dir + '_app'
    if not os.path.exists(out_app):
        os.makedirs(out_app)
    print('\tStoring particles grouped by tomograms in directory: ' + out_app)
    for tomo in list_3.get_tomo_list():
...
if in_wspace is None:
    ...
    for tkey, ltomo_1 in zip(iter(list_1.get_tomos().keys()), iter(list_1.get_tomos().values())):
        ...
        # if os.path.split(tkey)[1] == 'syn_14_24_bin2_rot_crop2_seg.fits':
        #     print 'jol'
        tomo_count += 1
        ...
        cfinder = ColumnsFinder(ltomo_1, ltomo_2, ltomo_3, ana_col_rad_v, ana_aln_dst_v,
                                    nn_1=ana_nn_1, nn_2=ana_nn_2, nn_3=ana_nn_3)
        ...
        tomos_acol[tkey] = cfinder.get_area_columns(mode='cyl', rad=ana_col_rad_v)
        if len(tomos_acol) > 0:
        ...
        for i in range(p_nsims):
            ...
            hold_area, hold_acols = cfinder2.get_area_tomo(), cfinder2.get_area_columns(mode='cyl', rad=ana_col_rad_v)
            ...
            hold_area, hold_acols = cfinder3.get_area_tomo(), cfinder3.get_area_columns(mode='cyl', rad=ana_col_rad_v)
    ...
    tomos_nc_sims2, tomos_den_sims2, tomos_occ_sims2, tomos_area_sims2, tomos_acol_sims2 = wspace[30], wspace[31], \
                                                                                           wspace[32], wspace[33], \
                                                                                           wspace[34]

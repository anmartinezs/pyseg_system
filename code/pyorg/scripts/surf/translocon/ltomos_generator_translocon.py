...
# Input STAR file
in_star = ROOT_PATH + '/in/in_ltomos_FTR_dp8_dec0.5.star'

# Input STAR for with the sub-volumes segmentations
in_seg = ROOT_PATH + '/in/in_seg_FTR_dp8_dec0.5.star'

# Output directory
out_dir = ROOT_PATH + '/ltomos/FTR_dp8_dec0.5_sxy'
out_stem = 'FTR_dp8' # 'pre'
...
sg_bm = 'center'
sg_pj = False
sg_origin = 4 # If not None, subtomoavg shiftings are considered, then scale factor from picked particle to subtomograms
sg_voi_mask = True
# Directory for VOI memory maps (to save memory and only used if sg_voi_mask==True)
sg_dmap_path = ROOT_PATH + '/ltomos/FTR_dp8_dec0.5_sxy/dmaps'
...
if sg_origin is not None:
    print('\t\t-Scale factor for subtomo-averaging origins: ' + str(sg_origin))
if sg_voi_mask:
...
try:
    ...
    sys.exit(-1)
# print '\t\tSet segmentation to bin 2 since segmentation has been done at this resolution in TRANSLOCON project...'
# for row in range(star_seg.get_nrows()):
#     hold_x, hold_y, hold_z = star_seg.get_element('_psSegOffX', row), star_seg.get_element('_psSegOffX', row), \
#                                 star_seg.get_element('_psSegOffX', row)
#     star_seg.set_element('_psSegOffX', row, 2.*hold_x), star_seg.set_element('_psSegOffX', row, 2.*hold_y), \
#         star_seg.set_element('_psSegOffX', row, 2.*hold_z)
...
for star_row in range(star.get_nrows()):
    ...
    # print '\t\tSet particles to bin 2 since segmentation has been done at this resolution in TRANSLOCON project...'
    # for row in range(star_part.get_nrows()):
    #     hold_x, hold_y, hold_z = star_part.get_element('_rlnCoordinateX', row), star_part.get_element('_rlnCoordinateY', row), \
    #                              star_part.get_element('_rlnCoordinateZ', row)
    #     star_part.set_element('_rlnCoordinateX', row, 2.*hold_x), star_part.set_element('_rlnCoordinateY', row, 2.*hold_y),\
    #     star_part.set_element('_rlnCoordinateZ', row, 2.*hold_z)

...
# Input STAR file
in_star_ref = ROOT_PATH + '/stat/ltomos/ribo_rln_aln2_ribo_org/ribo_org_ssup_8_min_50_ltomos.star'  #
in_ref_short_key = '0'
in_star = ROOT_PATH + '/stat/ltomos/ribo_rln_aln2_ost_org/ribo_org_ssup_8_min_50_ltomos.star'  #
in_wspace = None  # ROOT_PATH + '/stat/uni/try2_R/bi_F_sph_ssup_20_min_5/bi_FT_50_5_200_3_sim_10_fig_wspace.pkl' # (Insert a path to recover a pickled workspace instead of doing a new computation)

# Output directory
out_dir = ROOT_PATH + '/stat/uni/ribo_rln_aln2_all_org/all_org_ssup_8_min_50_npr'
out_stem = 'bi_ribo_20_5_200_3_sim_1_npr'  # ''uni_sph_4_60_5'

# Analysis variables
ana_res = 1.048  # nm/voxel
ana_rg = np.arange(5, 200, 3)  # in nm
ana_shell_thick = 20  # 20 # 10
ana_loc = False
ana_border = True
...
p_per = 5  # %
# Particle surface
p_vtp = ROOT_PATH + '/../vtp/class_T_b4_cent_swap.vtp'
...
if in_wspace is None:
    ...
    for lkey in lists_hash.values():
        ...
        for tkey in tomos_hash.keys():
            ...
            if not ref_tomo.is_active():
                ref_tomo.set_active(on=True)
            ...
            if ana_loc:
                hold_num, hold_den = ref_tomo.simulate_bi_2nd_order(ltomo, p_nsims, ModelCSRV, part_vtp,
                                                                    ana_rg_v, thick=ana_shell_thick_v,
                                                                    border=ana_border,
                                                                    conv_iter=ana_conv_iter, max_iter=ana_max_iter,
                ...
                hold_arr = ref_tomo.simulate_bi_2nd_order(ltomo, p_nsims, ModelCSRV, part_vtp,
                                                          ...
                                                          npr=ana_npr, npr_model=ana_npr_model,
                                                          tmp_folder=tmp_sim_folder,
                                                          verbose=pt_sim_v)
            if hold_arr is not None:
                for arr in hold_arr:
            ...
            if ref_tomo.is_active():
                ref_tomo.set_active(on=False)
            if ltomo.is_active():
                ltomo.set_active(on=False)
...
for lkey, tlist in zip(lists_exp.iterkeys(), lists_exp.itervalues()):
    ...
    if lkey == '0':
        plt.plot(ana_rg, exp_med, color='cyan', linestyle='-', label=lkey, linewidth=2.0)
    elif lkey == '1':
        plt.plot(ana_rg, exp_med, color='yellow', linestyle='-', label=lkey, linewidth=2.0)
...
for lkey, tlist in zip(lists_exp.iterkeys(), lists_exp.itervalues()):
    ...
    if lkey == '0':
        # plt.plot(ana_rg, exp_med, color='cyan', linestyle='-', label=lkey, linewidth=2.0)
        plt.plot(ana_rg, p_values, color='cyan', linestyle='-', label=lkey, linewidth=2.0)
    elif lkey == '1':
        # plt.plot(ana_rg, exp_med, color='yellow', linestyle='-', label=lkey, linewidth=2.0)
        plt.plot(ana_rg, p_values, color='yellow', linestyle='-', label=lkey, linewidth=2.0)

...
ROOT_PATH = '/fs/pool/pool-EMpub/4Antonio/fromQiang/tric_stat_test' # '/fs/pool/pool-ruben/Qiang/TriC_All/distance'

# Input STAR file
in_star = ROOT_PATH + '/ltomos/all_ltomos.star' # '/ltomos/all_ltomos.star'
in_wspace = None #ROOT_PATH + '/out/uni/test_5_700_3_sim_12_wspace.pkl' # None # (Insert a path to recover a pickled workspace instead of doing a new computation)

# Output directory
out_dir = ROOT_PATH + '/out/uni_shell'
out_stem = 'test_5_150_1_3_sim_3'

# Analysis variables
ana_res = 1.368 # nm/voxel
ana_rg = np.arange(5, 150, 1) # np.arange(5, 700, 3) # in nm_lsim
ana_shell_thick = 3 # 10
ana_conv_iter = 200
ana_max_iter = 100000
ana_npr = 3 # 10 # None means Auto
ana_npr_model = 1 # 10
...
# Particle surface
p_vtp = ROOT_PATH + '/in/sph_rad5_cent.vtp'
...
for lkey, tlist in zip(iter(lists_exp.keys()), iter(lists_exp.values())):
    ...
    plt.close()
    out_csv = out_lists_dir + '/uni_lists_' + str(lkey) + '.csv'
    with open(out_csv, 'w') as csv_file:
        writer = csv.DictWriter(csv_file, dialect=csv.excel_tab, fieldnames=('X', 'Yel', 'Yem', 'Yeh', 'Ysl', 'Ysm', 'Ysh'))
        writer.writeheader()
        for i in range(ana_rg.shape[0]):
            writer.writerow({'X':ana_rg[i], 'Yel':exp_low[i], 'Yem':exp_med[i], 'Yeh':exp_high[i], 'Ysl':ic_low[i], 'Ysm':ic_med[i], 'Ysh':ic_high[i]})
...
if do_pval:
    ...
    plt.close()
    out_p_csv = out_lists_dir + '/pvals_lists.csv'
    with open(out_p_csv, 'w') as p_csv_file:
        writer = csv.DictWriter(p_csv_file, dialect=csv.excel_tab, fieldnames=list(p_dic.keys()))
        writer.writeheader()
        for i in range(ana_rg.shape[0]):
            hold_dic = dict()
            for lkey, arr in zip(iter(p_dic.keys()), iter(p_dic.values())):
                hold_dic[lkey] = arr[i]
            writer.writerow(hold_dic)

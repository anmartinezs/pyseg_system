...
# Input STAR file with particles
in_star = ROOT_PATH + '/in/in_particles.star'
in_stem_rep = None # ('.', '.') # Replace stem 0 by 1 in particles lists

# Input STAR for pairing particles with the sub-volume segmentations
in_seg = ROOT_PATH + '/in/in_seg_all.star'

# Output directory
out_dir = ROOT_PATH + '/ltomos'
out_stem = 'all'
...
sg_dmap_path = ROOT_PATH + '/ltomos/all/dmaps'
...
for tomo_row in range(star_seg.get_nrows()):
    ...
    print 'SEGMENTATION PATH:' + seg_str
    if sg_voi_mask:
        voi = tomo == sg_lbl

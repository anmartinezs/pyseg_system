...
# Input STAR file
in_star = ROOT_PATH + '/nrt/ltomos_k4_gather_premb_mask/k4_gather_premb_mask_ltomos.star' # '/ref_a3/ltomos/pre_ltomos.star' # '/ref_a2/ltomos/pre_ref_a2_12_ltomos_hold.star'
in_wspace = None # ROOT_PATH + '/nrt/uni_1st_k4_gather_tomos/uni_sim_20_wspace.pkl' # (Insert a path to recover a pickled workspace instead of doing a new computation)

# Output directory
out_dir = ROOT_PATH + '/nrt/uni_1st_k4_gather_tomos'
out_stem = 'uni_sim_100' # ''uni_sph_4_60_5'
...
for tkey, ltomo in zip(iter(tomos_exp_fdsts.keys()), iter(tomos_exp_fdsts.values())):
    ...
    for lkey, farr in zip(iter(tomos_exp_fdsts[tkey].keys()), iter(tomos_exp_fdsts[tkey].values())):
        ...
        plt.xlabel('Scale (nm)')
        plt.plot(cdf_bins[:len(cdf_jvals)], cdf_jvals, color=lists_color[lkey], label=lkey)
        plt.legend(loc=4)
        plt.plot(cdf_bins[:len(cdf_sim_jlow)], cdf_sim_jlow, 'k--')
        plt.plot(cdf_bins[:len(cdf_sim_jmed)], cdf_sim_jmed, 'k')
...
for lkey, ltomo in zip(iter(lists_exp_fdsts.keys()), iter(lists_exp_fdsts.values())):
    ...
    plt.ylabel('Function-J')
    plt.xlabel('Scale (nm)')
    plt.plot(cdf_bins[:len(cdf_jvals)], cdf_jvals, color=lists_color[lkey])
    plt.plot(cdf_bins[:len(cdf_sim_jlow)], cdf_sim_jlow, 'k--')
    plt.plot(cdf_bins[:len(cdf_sim_jmed)], cdf_sim_jmed, 'k')
    plt.plot(cdf_bins[:len(cdf_sim_jhigh)], cdf_sim_jhigh, 'k--')
    plt.tight_layout()

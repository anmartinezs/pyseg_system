...
# Input STAR file
in_star = ROOT_PATH + '/stat/ltomos/try2_FT/try2_ssup_8_min_50_ltomos.star' # '/ref_a3/ltomos/pre_ltomos.star' # '/ref_a2/ltomos/pre_ref_a2_12_ltomos_hold.star'
in_wspace = ROOT_PATH + '/stat/uni/try2_FT/uni_1st/uni_1st_sim_10_wspace.pkl' # (Insert a path to recover a pickled workspace instead of doing a new computation)

# Output directory
out_dir = ROOT_PATH + '/stat/uni/try2_FT/uni_1st'
out_stem = 'uni_1st_sim_10_fig2' # ''uni_sph_4_60_5'
...
# Particle surface
p_vtp = ROOT_PATH + '/../vtp/class_R_dp8_b4_cent_dec0.5.vtp'
...
for lkey, ltomo in zip(iter(lists_exp_fdsts.keys()), iter(lists_exp_fdsts.values())):
    ...
    if len(cdf_sim_jlow) < len(cdf_sim_jhigh):
        plt.fill_between(cdf_bins[:len(cdf_sim_jlow)]*crf, cdf_sim_jlow[:len(cdf_sim_jlow)], cdf_sim_jhigh[:len(cdf_sim_jlow)], alpha=0.5, color='gray', edgecolor='w')
        x_lim_max = cdf_bins[len(cdf_sim_jlow)]
    else:
        plt.fill_between(cdf_bins[:len(cdf_sim_jhigh)] * crf, cdf_sim_jlow[:len(cdf_sim_jhigh)],
                         cdf_sim_jhigh[:len(cdf_sim_jhigh)], alpha=0.5, color='gray', edgecolor='w')
        x_lim_max = cdf_bins[len(cdf_sim_jlow)]

...
# Input STAR file
in_star_ref = ROOT_PATH + '/ves_40/ltomos_lap/lap_ltomos.star'
key_ref = 'tether'
in_star_1 = ROOT_PATH + '/pre/ref_nomb_1_clean/ltomos_pre_premb_gather_mask/pre_premb_gather_mask_ltomos.star'
key_1 = 'pre'
in_star_2 = ROOT_PATH + '/pst/nrt/ltomos_k4_gather_premb_mask/k4_gather_premb_mask_ltomos.star'
key_2 = 'pst'
...
key_3 = 'ampar'
in_star_4 = ROOT_PATH + '/pst/ampar_vs_nmdar/org/ltomos/ltomos_ampar_nmdar_premb_mask/ampar_nmdar_premb_gather_mask_ltomos.star'
key_4 = 'nmdar'
in_tethers_csv = ROOT_PATH + '/pre/ref_nomb_1_clean/py_scripts/syn_num_tethers.csv'
in_wspace = None # ROOT_PATH + '/pst/ampar_vs_nmdar/org/aln/aln_pst/aln_5_25_5_sim_200_global_maxd_20_wspace.pkl' # (Insert a path to recover a pickled workspace instead of doing a new computation)

# Output directory
out_dir = ROOT_PATH + '/pst/ampar_vs_nmdar/org/aln/aln_tether'
out_stem = 'aln_5_35_5_sim_20_global_maxd_15_noswitch' # ''uni_sph_4_60_5'
...
ana_rg = np.arange(5, 35, 5) # np.arange(4, 100, 2) # in nm
...
print '\t\t-Key: ' + key_2
print '\tInput analysis STAR file 4: ' + str(in_star_4)
print '\t\t-Key: ' + key_2
...
star_1, star_2, star_3, star_4, star_ref = sub.Star(), sub.Star(), sub.Star(), sub.Star(), sub.Star()
try:
...
ltomos_pkl = star_4.get_element('_psPickleFile', 1)
...
if pre_ssup is not None:
    ...
    list_4.scale_suppression(pre_ssup_v)
    ...
    for li, lkey in enumerate(lists_hash.values()):
        ...
        for lt, tkey in enumerate(tomos_hash.keys()):
            ...
            print '\t\t\t\t*Processing tomogram (list ' + str(li + 1) + ' of ' + str(
                len(lists_hash.values())) + ', tomo ' + str(lt + 1) + ' of ' + str(len(tomos_hash.keys())) + ') : ' + \
            ...
            tomos_np_ref[lkey][tkey] = ref_tomo.get_num_particles()
            ...
            if ana_global:
                hold_arr_1, hold_arr_2 = ref_tomo.compute_bi_2nd_order(ltomo, ana_rg_v, thick=ana_shell_thick_v,
            ...
            if ana_global:
                hold_arr_1, hold_arr_2 = ref_tomo.simulate_bi_2nd_order(ltomo, p_nsims, ModelCSRV, part_vtp,
                                                                    ana_rg_v, thick=ana_shell_thick_v, border=ana_border,
                                                                    conv_iter=ana_conv_iter, max_iter=ana_max_iter,
                                                                    npr=ana_npr, npr_model=ana_npr_model, switched=False, out_sep=2,
...
for tkey, ltomo in zip(tomos_aln.iterkeys(), tomos_aln.itervalues()):
    if len(ltomo.keys()) <= 0:
        continue
    tkey_short = os.path.splitext(os.path.split(tkey)[1])[0]
    ...
    ax2 = ax1.twinx()
    # plt.title('Univariate 2nd order for ' + tkey_short)
    ax1.set_xlabel('Tether centroid alignment (a.u.)')
    ax1.set_ylabel('proportion')
    ax2.set_ylabel('p-value')
    for lkey, arr in zip(tomos_aln[tkey].iterkeys(), tomos_aln[tkey].itervalues()):
        if lkey == key_1:
            ax1.bar(0.60, tomos_aln[tkey][lkey], BAR_WIDTH, color='blue', linewidth=2)
            ax1.bar(0.9, np.asarray(tomos_aln_sim[tkey][lkey]).mean(), BAR_WIDTH, color='blue', linewidth=2, hatch='\\')
            ax2.bar(1.20, compute_pval(tomos_aln[tkey][lkey], np.asarray(tomos_aln_sim[tkey][lkey])), BAR_WIDTH,
                    color='blue', linewidth=2, hatch='//')
        elif lkey == key_2:
            ax1.bar(1.60, tomos_aln[tkey][lkey], BAR_WIDTH, color='red', linewidth=2)
            ax1.bar(1.9, np.asarray(tomos_aln_sim[tkey][lkey]).mean(), BAR_WIDTH, color='red', linewidth=2, hatch='\\')
            ax2.bar(2.20, compute_pval(tomos_aln[tkey][lkey], np.asarray(tomos_aln_sim[tkey][lkey])), BAR_WIDTH,
                    color='red', linewidth=2, hatch='//')
    plt.xticks((1, 2), ('PRE', 'PST'))
...
for lkey, tlist in zip(lists_exp.iterkeys(), lists_exp.itervalues()):
    ...
    if ana_global:
        if lkey == key_1:
            plt.plot(ana_rg, np.asarray(tlist).mean(axis=0), color='blue', linewidth=2, linestyle='-', label='TETHER$\Rightarrow$PRE')
        elif lkey == key_2:
            plt.plot(ana_rg, np.asarray(tlist).mean(axis=0), color='black', linewidth=2, linestyle='-', label='TETHER$\Rightarrow$PST')
    else:
...
print '\t\t-Plotting alignment metric...'
plt.subplots()
plt.xlabel('Tether alignment (a.u.)')
plt.ylabel('proportion')
for lkey in lists_aln.iterkeys():
    aln_sim_arr = np.asarray(lists_aln_sim[lkey])
    ic_low, ic_med, ic_high = np.percentile(aln_sim_arr, 5), np.percentile(aln_sim_arr, 50), np.percentile(aln_sim_arr, 95)
    if lkey == key_1:
        plt.bar(0.70, lists_aln[lkey], BAR_WIDTH, color='blue', linewidth=2)
        plt.bar(1.1, ic_med, BAR_WIDTH, color='gray', linewidth=2)
        plt.errorbar(1.2, ic_med, yerr=np.asarray([[ic_med-ic_low, ic_high-ic_med],]).reshape(2,1), ecolor='k', elinewidth=4, capthick=4, capsize=8)
    elif lkey == key_2:
        plt.bar(1.70, lists_aln[lkey], BAR_WIDTH, color='red', linewidth=2)
        plt.bar(2.1, ic_med, BAR_WIDTH, color='gray', linewidth=2)
        plt.errorbar(2.2, ic_med, yerr=np.asarray([[ic_med-ic_low, ic_high-ic_med],]).reshape(2,1), ecolor='k', elinewidth=4, capthick=4, capsize=8)
    elif lkey == key_3:
        plt.bar(2.70, lists_aln[lkey], BAR_WIDTH, color='black', linewidth=2)
        plt.bar(3.1, ic_med, BAR_WIDTH, color='gray', linewidth=2)
        plt.errorbar(3.2, ic_med, yerr=np.asarray([[ic_med-ic_low, ic_high-ic_med],]).reshape(2,1), ecolor='k', elinewidth=4, capthick=4, capsize=8)
    elif lkey == key_4:
        plt.bar(3.70, lists_aln[lkey], BAR_WIDTH, color='lightsalmon', linewidth=2)
        plt.bar(4.1, ic_med, BAR_WIDTH, color='gray', linewidth=2)
        plt.errorbar(4.2, ic_med, yerr=np.asarray([[ic_med-ic_low, ic_high-ic_med],]).reshape(2,1), ecolor='k', elinewidth=4, capthick=4, capsize=8)
plt.xticks((1, 2, 3, 4), ('PRE', 'PST', 'AMPAR', 'NMDAR'))
plt.xlim(0.5, 4.5)
plt.ylim(0, 1)
plt.tight_layout()
if fig_fmt is None:
    ...
    plt.savefig(out_lists_dir + '/align_tether.png', dpi=300)
plt.close()

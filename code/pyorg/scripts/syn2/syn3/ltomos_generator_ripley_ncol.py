...
# Input STAR file
in_star = ROOT_PATH + '/ripley/ncol/in/all_ncol_ltomos.star' # '/org/col/in/all_xd5nm_ltomos.star' # '/org/col/in/all_test_ltomos.star'

# Input STAR for with the sub-volumes segmentations
in_seg = '/fs/pool/pool-lucic2/antonio/workspace/psd_an/in/syn_seg_no_l14_gap.star' # '/fs/pool/pool-lucic2/antonio/workspace/psd_an/in/syn_seg_no_l14_gap_zeros_fits.star' # '/fs/pool/pool-lucic2/antonio/workspace/psd_an/in/syn_seg_no_l14_gap_zeros_fits.star'

# Output directory
out_dir = ROOT_PATH + '/ripley/ncol/ltomos/ltomos_ncol' # '/org/ncol/ltomos/ltomos_ncol_tmin3_min5' # '/org/col/ltomos/ltomos_all_scol_test'
...
# Particles rescaling
res_f = 0.842 / 0.684 # nm/voxel
in_tomo_res = ROOT_PATH + '/rec/syn_scale_tomos.star'
...
for tomo_row in range(star_seg.get_nrows()):
    ...
    if tomos_to_resize is not None:
        tkey_hold = os.path.split(tomo_path)[1].split('_')
        tkey_stem = tkey_hold[1] + '_' + tkey_hold[2]
        if mic_str in tkey_stem:
            tomo = sp.ndimage.zoom(tomo, res_f, order=0)
    if sg_voi_mask:
        voi = tomo == sg_lbl
        vois[seg_str] = voi

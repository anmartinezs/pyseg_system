...
# Input STAR file
in_star = ROOT_PATH + '/ripley/ncol/ltomos/ltomos_ncol/all_ncol_ltomos_surf.star' # '/org/col/ltomos/ltomos_preb_tether_pstb_xd5nm_amin10/all_preb_tether_pstb_xd5nm_amin10_ltomos_surfs.star' # '/org/pst/ltomos/ltomos_all_pst_pre_ss15px_v6/all_pst_pre_ltomos_surf.star'
in_wspace = None # ROOT_PATH + '/ripley/ncol/uni_2nd/all_ncol_ss7.31px_sim200_wspace.pkl' # (Insert a path to recover a pickled workspace instead of doing a new computation)

# Output directory
out_dir = ROOT_PATH + '/ripley/ncol/uni_2nd/'
out_stem = 'all_ncol_ss7.31px_sim200' # 'all_preb_tether_pstb_xd5nm_amin10_sim10'
    ...
    for lkey, arr in zip(tomos_exp[tkey].iterkeys(), tomos_exp[tkey].itervalues()):
        ...
        if ana_shell_thick is None:
            ...
            if ana_rdf:
                ...
                plt.ylabel('Ripley\'s O')
for lkey, vals in zip(hold_sim_vals.iterkeys(), hold_sim_vals.itervalues()):
...
for lkey, tlist in zip(lists_den.iterkeys(), lists_den.itervalues()):
    ...
    ticks[lkey] = count
    count += 1
plt.xticks(ticks.values(), ticks.keys())
plt.tight_layout()
if fig_fmt is None:
    plt.show(block=True)
else:

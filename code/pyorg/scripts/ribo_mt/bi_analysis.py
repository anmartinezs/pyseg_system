...
ROOT_PATH = '/fs/pool/pool-plitzko/Saikat/ribo_org'

# Input STAR file
in_star_ref = ROOT_PATH + '/test_1/ltomos/fmm_mt_ribo_proj/run1_ltomos.star'
in_ref_short_key = '0'
in_star = ROOT_PATH + '/test_1/ltomos/fmm_mt_ribo_proj/run1_ltomos.star' #
in_wspace = None # (Insert a path to recover a pickled workspace instead of doing a new computation)

# Output directory
out_dir = ROOT_PATH + '/test_1/bi/fmm_mt_ribo_proj_switch'
out_stem = 'test_8_300_5_sim_20' # ''uni_sph_4_60_5'
...
# Particle surface
p_vtp = ROOT_PATH + '/test_1/in/ribo_bin4_dec0.8.vtp' # '/../vtp/class_F_dp8_b4_cent_dec0.9_nomb.vtp'
        ...
        for tkey in tomos_hash.keys():
            ...
            if ana_loc:
                hold_num, hold_den = ref_tomo.simulate_bi_2nd_order(ltomo, p_nsims, ModelCSRV, part_vtp,
                                                                    ...
                                                                    conv_iter=ana_conv_iter, max_iter=ana_max_iter,
                                                                    npr=ana_npr, npr_model=ana_npr_model, switched=p_switch,
                                                                    out_sep=True, tmp_folder=tmp_sim_folder,
                                                                    verbose=pt_sim_v)
                if (hold_den is None) or (hold_den is None):
                    hold_arr = None
                else:
                ...
                hold_arr = ref_tomo.simulate_bi_2nd_order(ltomo, p_nsims, ModelCSRV, part_vtp,
                                                          ...
                                                          npr=ana_npr, npr_model=ana_npr_model, switched=p_switch, tmp_folder=tmp_sim_folder,
                                                          verbose=pt_sim_v)
            if hold_arr is not None:
                for arr in hold_arr:

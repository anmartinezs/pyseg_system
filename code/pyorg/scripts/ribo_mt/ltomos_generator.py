...
# Output directory
out_dir = ROOT_PATH + '/ltomos/v1_m50' # '/stat/ltomos/trans_run2_test_swapxy'
out_stem = 'v1_m50' # 'pre'

pt_res = 1.792 # nm/vx - resolution
...
sg_origins = (8, 8) # If not None, subtomoavg shiftings are considered,
...
pt_ss_ref = ('0', '1') # a tuple with the sorted preference for crossed patterns scaled suppresion, if None deactivated
pt_ss_ref_dst = 6 # for using different scale-suppression distances for crossed scale suppression
pt_min_parts = 50 # Tomograms with less particles are removed
...
for star_row in range(star.get_nrows()):
    ...
    for part_row in range(star_part.get_nrows()):
        ...
        # Initialization
        seg_str = star_part.get_element('_psSegImage', part_row)
        try:
            seg_row = seg_dic[seg_str]
        except KeyError:
            print('WARNING: Segmentation image not found: ' + mic_str)
            continue

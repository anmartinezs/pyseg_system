...
def manifold3d_from_vtu_to_img(filename, outputdir=None, format='vti', transpose=False,
    ...
    if not filename.endswith('vtu'):
        raise pyseg.pexceptions.PySegInputWarning(expr='numpy_to_vti', msg='Only input format \'vtu\' is allowed.')
    ...
    if cell_data is None:
        ...
        raise pyseg.pexceptions.PySegInputError(expr='manifold3d_from_vtu_to_img', msg=error_msg)
    ...
    if format == 'vti':
        ...
        raise pyseg.pexceptions.PySegInputWarning(expr='manifold3d_from_vtu_to_img', msg='Only \'vti\', \'mrc\' and \'em\' formats are allowed')
...
def gen_surface(tomo, lbl=1, mask=True, purge_ratio=1, field=False, mode_2d=False, verbose=False):
    ...
    if isinstance(tomo, str):
        ...
        if fext == '.fits':
            ...
            raise pyseg.pexceptions.PySegInputError(expr='gen_surface', msg=error_msg)
...
def gen_surface_cloud(tomo, lbl=1, purge_ratio=1, cloud=False, verbose=False):
    ...
    if isinstance(tomo, str):
        ...
        error_msg = 'Input must be either a file name or a ndarray.'
        raise pyseg.pexceptions.PySegInputError(expr='gen_surface_cloud', msg=error_msg)
...
def gauss_smooth_vfield(cloud, sigma, size=None):
    ...
    if (len(tomo_in.shape) != 4) or (tomo_in.shape[3] != 3):
        ...
        raise pyseg.pexceptions.PySegInputError(expr='gauss_smooth_vfield', msg=error_msg)
...
def save_vtp(poly, fname):
    ...
    if writer.Write() != 1:
        ...
        raise pyseg.pexceptions.PySegInputError(expr='save_vtp', msg=error_msg)
...
def save_vti(image, fname, outputdir):
    ...
    if writer.Write() != 1:
        ...
        raise pyseg.pexceptions.PySegInputError(expr='save_vti', msg=error_msg)
...
def crop_lbl_tomo(vol, seg, lbl=1, off=0):
    ...
    if vol.shape != seg.shape:
        ...
        raise pyseg.pexceptions.PySegInputError(expr='crop_lbl_tomo', msg=error_msg)
...
class TypesConverter(object):
    ...
    def numpy_to_gt(din, array=False):
        ...
        if (not isinstance(din, type)) and (not isinstance(din, numpy.dtype)):
            ...
            raise pyseg.pexceptions.PySegInputError(expr='numpy_to_gt (TypesConverter)',
                                                    msg=error_msg)
    ...
    def vtk_to_gt(din, array=False):
        ...
        if not isinstance(din, vtk.vtkDataArray):
            error_msg = 'vtkDataArray object required as input.' % din
            raise pyseg.pexceptions.PySegInputError(expr='vtk_to_gt (TypesConverter)', msg=error_msg)
    ...
    def gt_to_vtk(din):
        ...
        if not isinstance(din, str):
            error_msg = 'str object required as input.' % din
            raise pyseg.pexceptions.PySegInputError(expr='gt_to_vtk (TypesConverter)', msg=error_msg)

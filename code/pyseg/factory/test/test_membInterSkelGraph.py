...
from pyseg.factory import MembInterSkelGraph
...
class TestMembInterSkelGraph(TestCase):
    ...
    def test_Class(self):
        ...
        mb_skel = MembInterSkelGraph(skel, manifolds, density, seg)
        ...
        writer.SetFileName(DATA_DIR + '/mb_inter_graph.vtp')
        ...
        writer = vtk.vtkXMLPolyDataWriter()
        writer.SetFileName(DATA_DIR + '/mb_inter_sgraph.vtp')
        vtk_ver = vtk.vtkVersion().GetVTKVersion()
        if int(vtk_ver[0]) < 6:
        ...
        segs_img.write(file=DATA_DIR+'/skel_seg_inter_density.mrc', fileFormat='mrc')
        writer = vtk.vtkXMLImageDataWriter()
        writer.SetFileName(DATA_DIR + '/skel_seg_inter_lbl.vti')
        vtk_ver = vtk.vtkVersion().GetVTKVersion()
        if int(vtk_ver[0]) < 6:

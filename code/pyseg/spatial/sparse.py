...
class CSRSimulator(Simulator):
    ...
    def gen_rand_in_mask(self, n, mask):
        ...
        # Random generation loop
        ntries = 0
        mx_ntries = NTRIES_F * generator.get_initial_tries()
        npts = 0
        coords = -1. * np.ones(shape=(n, 3), dtype=np.float32)
        while (npts < n) and (ntries < mx_ntries):
            # Coordinates
...
class LinkerSimulator(Simulator):
    ...
    def __init__(self, leng, nc, temp, nrots=1000, no_over=False, len_sg=0.):
        ...
        if (not isinstance(temp, np.ndarray)) or (len(temp.shape) != 3):
            ...
            raise ps.pexceptions.PySegInputError(expr='__init__ (LinkerSimulator)', msg=error_msg)
        self.__temp = np.asarray(temp, dtype=np.float32)
        self.__nrots = int(nrots)
        if self.__nrots <= 0:
            error_msg = 'The number or random rotation must be greater than zero!'
...
class RubCSimulation(Simulator):
    ...
    def __init__(self, res, temp):
        ...
        if res <= 0:
            ...
            raise ps.pexceptions.PySegInputError(expr='__init__ (SinSimulator)', msg=error_msg)
        self.__res = res
        if (not isinstance(temp, np.ndarray)) or (len(temp.shape) != 3):
            error_msg = 'Particle distance must be a 3d numpy array!'
            raise ps.pexceptions.PySegInputError(expr='__init__ (LinkerSimulator)', msg=error_msg)
...
class AggSimulator(object):
    ...
    def __init__(self, res, temp, l_len_l, l_len_h, agg_sz, t, nrots=1000, max_rad=None, rig_sd=None):
        ...
        if (not isinstance(temp, np.ndarray)) or (len(temp.shape) != 3):
            error_msg = 'Particle distance must be a 3d numpy array!'
            raise ps.pexceptions.PySegInputError(expr='__init__ (NetSimulator)', msg=error_msg)
        self.__temp = np.asarray(temp, dtype=np.float32)
    ...
    def __gen_instance(self, npart, mask, temp):
        ...
        while (len(coords) < npart) and (curr_try < max_tries):
            ...
            hold_coords, hold_tries = self.__gen_aggregate(hold_np, max_tries-curr_try, hold_mask, hold_mask_pot,
                                                           global_gen)
            # ps.disperse_io.save_numpy(hold_mask, '/home/martinez/workspace/stat/pyrenoid/net_high_res/hold.mrc')
        ...
        # ps.disperse_io.save_numpy(hold_mask_pot, '/home/martinez/workspace/stat/pyrenoid/net_high_res/hold_tomo_pot.mrc')
    ...
    def __gen_rnd_rots(self):
        for i in range(self.__nrots):
            ...
            # ps.disperse_io.save_numpy(self.__temp, '/home/martinez/workspace/stat/pyrenoid/net_high_res/hold_temp.mrc')
            # ps.disperse_io.save_numpy(self.__ptemp, '/home/martinez/workspace/stat/pyrenoid/net_high_res/hold_ptemp.mrc')
            # ps.disperse_io.save_numpy(self.__rnd_rots[:, :, :, i], '/home/martinez/workspace/stat/pyrenoid/net_high_res/hold_rot.mrc')
            # ps.disperse_io.save_numpy(self.__rnd_prots[:, :, :, i], '/home/martinez/workspace/stat/pyrenoid/net_high_res/hold_prot.mrc')
    ...
    def __new_pot_particle(self):
        ...
        # ps.disperse_io.save_numpy(self.__agg_mask, '/home/martinez/workspace/stat/pyrenoid/net_high_res/hold_agg_mask.mrc')
        # ps.disperse_io.save_numpy(self.__agg_pot, '/home/martinez/workspace/stat/pyrenoid/net_high_res/hold_pot_mask.mrc')
        # ps.disperse_io.save_numpy(loc_mask, '/home/martinez/workspace/stat/pyrenoid/net_high_res/hold_loc_mask.mrc')
        ...
        while n_tries < max_tries:
            ...
            if not check_bin_sub_tomo(mask, seed_coord, rot_temp):
                ...
                while rlock and (len(queue) > 0):
                    ...
                    # ps.disperse_io.save_numpy(neigh_mask | self.__mask_sph, '/home/martinez/workspace/stat/pyrenoid/agg_test/hold.mrc')
                    ...
                    if curr_coord is not None:
                        ...
                        # ps.disperse_io.save_numpy(neigh_mask, '/home/martinez/workspace/stat/pyrenoid/agg_test/hold.mrc')
...
class UniStat(object):
    ...
    def to_2D(self, del_coord):
        ...
        if self.__temp is not None:
            ...
            raise ps.pexceptions.PySegInputError(expr='to_2D (UniStat)', msg=error_msg)
    ...
    def save_sparse(self, file_name, mask=False):
        if self.__temp is None:
            ...
            if mask:
                ...
                ps.disperse_io.save_numpy(hold_mask, file_name)
    ...
    def compute_G(self, max_d, bins=50):
        ...
        #     raise ps.pexceptions.PySegInputWarning(expr='compute_G (UniStat)', msg=error_msg)
    ...
    def compute_F(self, max_d, bins=50, f_npts=-1):
        if bins > self.__n_coords:
            error_msg = 'Number of bins ' + str(bins) + \
                        ' bigger than the number of coords ' + str(self.__n_coords)
            raise ps.pexceptions.PySegInputError(expr='compute_F (UniStat)', msg=error_msg)
        if f_npts < 0:
    ...
    def simulate_F(self, max_d, bins=50, f_npts=-1, n_sim=100, p=None, get_sim=False):
        ...
        #     error_msg = 'Number of bins ' + str(bins) + \
        #                 ' bigger than the number of coords ' + str(self.__n_coords)
        #     raise ps.pexceptions.PySegInputError(expr='simulate_F (UniStat)', msg=error_msg)
    ...
    def analyze_W(self, rads, bins=50, block=True, out_file=None, legend=False, pimgs=False):
        ...
        if (not hasattr(rads, '__len__')) and (len(rads) > 0):
            ...
            raise ps.pexceptions.PySegInputError(expr='analyze_W (UniStat)', msg=error_msg)
    ...
    def plot_points(self, block=True, out_file=None):
        ...
        if out_file is not None:
            ...
            if f_ext == '.vtp':
                ps.disperse_io.save_vtp(self.to_vtp(), out_file)
            else:
...
class BiStat(object):
    ...
    def save_sparse(self, file_name, pattern=1):
        if pattern == 1:
            ps.disperse_io.save_numpy(self.__dense_1, file_name)
        else:
            ps.disperse_io.save_numpy(self.__dense_2, file_name)
    ...
    def compute_G(self, max_d, bins=50):
        if bins > self.__n_coords_2:
            ...
            raise ps.pexceptions.PySegInputError(expr='compute_G (BiStat)', msg=error_msg)
...
class PlotUni(object):
    ...
    def analyze_J(self, block=True, out_file=None, legend=False, p=None):
        ...
        if (self.__Gs is None) or (self.__Fs is None):
            ...
            raise ps.pexceptions.PySegInputError(expr='analyze_J (UniStat)', msg=error_msg)
        colors = cm.rainbow(np.linspace(0, 1, len(self.__unis)))
    ...
    def analyze_K(self, max_d, n_samp=50, n_sim=0, p=None, update=False, block=True, tcsr=False,
        ...
        if n_samp <= 2:
            ...
            raise ps.pexceptions.PySegInputError(expr='analyze_K (UniStat)', msg=error_msg)
    ...
    def analyze_NLD(self, shell_th, max_d, n_samp=50, n_sim=0, p=None, update=False, block=True,
        ...
        if n_samp <= 2:
            error_msg = 'Number of samples must be greater than 2'
            raise ps.pexceptions.PySegInputError(expr='analyze_NLD (UniStat)', msg=error_msg)
    ...
    def analyze_L(self, block=True, out_file=None, legend=False, gather=False, p=None, gstd=False):
        ...
        if self.__Ks is None:
            ...
            raise ps.pexceptions.PySegInputError(expr='analyze_L (UniStat)', msg=error_msg)
        colors = cm.rainbow(np.linspace(0, 1, len(self.__unis)))
    ...
    def analyze_I(self, block=True, out_file=None, legend=False, gather=False, p=None, gstd=False):
        ...
        if (self.__K_nums is None) or (self.__K_dems is None):
            error_msg = 'analyze_K() must be called before with the same input parameters.'
            raise ps.pexceptions.PySegInputError(expr='analyze_I (UniStat)', msg=error_msg)
        colors = cm.rainbow(np.linspace(0, 1, len(self.__unis)))
    ...
    def analyze_O(self, w=1, block=True, out_file=None, legend=False, gather=False, p=None, gstd=False):
        ...
        if self.__Ks is None:
            error_msg = 'analyze_K() must be called before with the same input parameters.'
            raise ps.pexceptions.PySegInputError(expr='analyze_O (UniStat)', msg=error_msg)
        colors = cm.rainbow(np.linspace(0, 1, len(self.__unis)))
    ...
    def analyze_D(self, w=1, block=True, out_file=None, legend=False, gather=False, p=None, gstd=False):
        ...
        if (self.__K_nums is None) or (self.__K_dems is None):
            error_msg = 'analyze_K() must be called before with the same input parameters.'
            raise ps.pexceptions.PySegInputError(expr='analyze_D (UniStat)', msg=error_msg)
        colors = cm.rainbow(np.linspace(0, 1, len(self.__unis)))
    ...
    def __compute_Gs(self, max_d, bins, gather=False):
        ...
        if gather:
            ...
            try:
                ...
                print(e.get_message())
                raise ps.pexceptions.PySegInputError('__compute_Gs (PlotUni)', e.get_message())
        else:
            for uni in self.__unis:
                try:
...
class PlotBi(object):
    ...
    def analyze_L(self, block=True, out_file=None, legend=False, gather=False, p=None, gstd=False):
        ...
        if self.__Ks is None:
            error_msg = 'analyze_K() must be called before with the same input parameters.'
            raise ps.pexceptions.PySegInputError(expr='analyze_L (BiStat)', msg=error_msg)
        colors = cm.rainbow(np.linspace(0, 1, len(self.__bis)))
    ...
    def analyze_O(self, w=1, block=True, out_file=None, legend=False, gather=False, p=None, gstd=False):
        ...
        if self.__Ks is None:
            error_msg = 'analyze_K() must be called before with the same input parameters.'
            raise ps.pexceptions.PySegInputError(expr='analyze_O (BiStat)', msg=error_msg)
        colors = cm.rainbow(np.linspace(0, 1, len(self.__bis)))
    ...
    def __compute_Gs(self, max_d, bins, gather=False):
        ...
        if gather:
            ...
            try:
                ...
                print(e.get_message())
                raise ps.pexceptions.PySegInputError('__compute_Gs (PlotBi)', e.get_message())
        else:
            for bi in self.__bis:
                try:

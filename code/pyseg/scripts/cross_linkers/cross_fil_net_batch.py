"""
    ...
    Script for filter a MbGraphMCF object by extracting a filament network with cross-linkers between to segmented
    regions

    Input:  - MbGraphMCF (pickle file)
            - Cross-linkders setting

    Output: - A filtered version of the input MbGraphMCF with the cross-linkers
            - A VTP file with the filament network
...
in_star = ROOT_PATH + '/graph/test/mts_clines_mts_mb_graph_nomt.star'

# Cross-linker sources and targets slice XML files
in_sources = ROOT_PATH + '/scripts/sources_shell.xml'
in_targets = ROOT_PATH + '/scripts/targets_shell.xml'

####### Output data

out_dir = ROOT_PATH + '/fils'
out_int_g = True
...
print 'Filtering a MbGraphMCF for tracking cross-linkers.'

...
slices = SynSliceSet(slices_file) # SliceSet(slices_file)
    ...
    for sl in slices.get_slices_list():
        ...
        try:
            if sl.get_cont():
                ...
                cloud, cloud_ids, mask, cloud_w = graph.get_cloud_mb_slice(sl, cont_mode=1, graph_gt=graph_gt,
                                                                           cont_prop=peak_prop)
                print '\t\t\t\tCurrent number of points: ' + str(len(cloud_ids))
            else:
                cloud, cloud_ids, mask = graph.get_cloud_mb_slice(sl, cont_mode=0, graph_gt=graph_gt)
                print '\t\t\t\tCurrent number of points: ' + str(len(cloud_ids))
        except ValueError:
        ...
        if peak_prop is not None:
            ...
            if sl.get_cont():
                ...
                # cloud_cc = graph.get_prop_values(peak_prop, cloud_ids)
                cloud_cc = np.ones(len(cloud_ids))
        ...
        for coord, pt_coord in zip(coords, coords_pt):
            ...
            coord_imod, pt_coord_imod = coord_imod - tomo_center, pt_coord_imod - tomo_center
            R = ps.globals.rot_mat_relion(float(in_rot[0]), float(in_rot[1]), float(in_rot[2]), deg=True)
            coord_imod = np.asarray(R*coord_imod.reshape(3,1), dtype=np.float32).reshape(3) + tomo_center
            pt_coord_imod = np.asarray(R * pt_coord_imod.reshape(3, 1), dtype=np.float32).reshape(3) + tomo_center
            ...
            coord_rln = np.asarray((coord[0], coord[1], coord[2]), dtype=np.float32)
            pt_coord_rln = np.asarray((pt_coord[0], pt_coord[1], pt_coord[2]), dtype=np.float32)
            coord_rln, pt_coord_rln = coord_rln + gcrop_off_rln, pt_coord_rln + gcrop_off_rln
            coord_rln, pt_coord_rln = coord_rln - tomo_center, pt_coord_rln - tomo_center
            R = ps.globals.rot_mat_relion(float(in_rot[0]), float(in_rot[1]), float(in_rot[2]), deg=True)
            coord_rln = np.asarray(R * coord_rln.reshape(3, 1), dtype=np.float32).reshape(3) + tomo_center
            pt_coord_rln = np.asarray(R * pt_coord_rln.reshape(3, 1), dtype=np.float32).reshape(3) + tomo_center

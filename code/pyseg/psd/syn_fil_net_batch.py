"""
    ...
    Input:  - SynGraphMCF (pickle file)
            - Filament network sources (slice in a XML file)
            - Filament network targets (slice in a XML file)
            - Filament geometrical parameters (input parameters)

    Output: - A filtered version of the input SynGraphMCF
...
in_star = ROOT_PATH + '/ex/syn/graphs_ves/syn_seg_ves_graph_hold_ves.star'

# Sources slice XML file
in_sources = ROOT_PATH+'/ex/syn/fils/ves/sources.xml'

# Targets slice XML file
in_targets = ROOT_PATH+'/ex/syn/fils/ves/targets.xml'

####### Output data

out_dir = ROOT_PATH+'/ex/syn/fils/ves'
out_int_g = True

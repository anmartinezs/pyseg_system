...
# $Id: vesicles.py 1461 2017-10-12 10:10:49Z vladan $
"""
...
class Vesicles(Groups):
    ...
    def addLinked(self, files, contactName='contacts'):
        ...
        for categ in self.values():
            categ.linked = [None] * len(categ.identifiers)

        # loop over categories
        for obj, categ, ident in db.data(category=self.keys()):
            ...
            for obs_ind, loc_ident in zip(range(len(self[categ].identifiers)),
    ...
    def getMeanConnectionLength(self, conn, categories=None, 
        ...
        for categ in categories:
            for ident in conn[categ].identifiers:
                ...
                sv_dict_mean = {}
                for ind, lengths in sv_dict.items():
        ...
        for categ in categories:
            ...
            for obs_ind in range(len(clusters[categ].ids)):
                ...
                for clust, clust_size \
                    ...
                    sizes.update(dict(zip(clust, [clust_size] * clust_size)))

...
int dsyevh3(float A[3][3], float Q[3][3], float w[3]);
int dsyevc3(float A[3][3], float w[3]);
inline void dsytrd3(float A[3][3], float Q[3][3], float d[3], float e[2]);
int dsyevq3(float A[3][3], float Q[3][3], float w[3]);
...
{ 
    ...
    if (nrhs != 6) { 
		mexErrMsgTxt("desyevhmex_single.cpp: Six input arguments required."); 
		return;
    } else if (nlhs > 12) {
		mexErrMsgTxt("desyevhmex_single.cpp: Too many output arguments."); 
		return;
    }
	...
	if ( (m!=mxGetM(prhs[1])) || (m!=mxGetM(prhs[2])) || (m!=mxGetM(prhs[3])) || 
		...
		mexErrMsgTxt("desyevhmex_single.cpp: Dimensions mismatch."); 
		return;
	}
    if ( (mxGetClassID(prhs[0])!=mxSINGLE_CLASS) || mxIsComplex(prhs[0]) || 
		...
		mexErrMsgTxt("desyevhmex_single.cpp: Requires double real data."); 
		return;
	} 
	...
	if (pthread_mutex_init( &mutex, NULL ) ){
		mexErrMsgTxt("desyevhmex_single.cpp: Error creating the mutex.");
		return;
	}
	...
	for (int i=0; i<nth; i++) {
		...
		if (pthread_create(&threads[i],NULL,&desyevh3stub,NULL)) {
			mexErrMsgTxt("desyevhmex_single.cpp: Error creating a thread.");
			return;
		}
	}
	...
	for (int i=0; i<nth; i++) {
		if (pthread_join(threads[i],NULL)) {
			mexErrMsgTxt("desyevhmex_single.cpp: Error waiting the thread termination.");
			return;
		}
	} 

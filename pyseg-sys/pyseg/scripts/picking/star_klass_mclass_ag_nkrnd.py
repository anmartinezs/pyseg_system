"""
    ...
    Unsupervised and deterministic classification test for HAC algorithm
...
# PCA+HAC classification
ag_ncomp = 21 # 18
ag_n_clusters_rg = [1, 200]
ag_linkage = 'ward'
...
if in_ext == '.pkl':
    ...
    print '\tOutput stem for HAC: ' + str(out_stem)
...
print '\tClustering: '
print '\t\t-Number of components: ' + str(ag_ncomp)
print '\t\tAgglomerative clustering classificiation settings: '
print '\t\t\t-Range for the number of clusters to find: ' + str(ag_n_clusters_rg)
...
try:
    evs = star_class.vectors_dim_reduction(n_comp=ag_ncomp, method='pca')
...
nks = np.random.randint(low=ag_n_clusters_rg[0], high=ag_n_clusters_rg[1], size=sim_n)
...
for i in range(sim_n):
    ...
    try:
        star_class.agglomerative_clustering(mode_in='vectors', n_clusters=nks[i], linkage=ag_linkage, knn=None,
                                            verbose=False)
    except ps.pexceptions.PySegInputError as e:
        print 'ERROR: Classification failed because of "' + e.get_message() + '"'
        print 'Terminated. (' + time.strftime("%c") + ')'

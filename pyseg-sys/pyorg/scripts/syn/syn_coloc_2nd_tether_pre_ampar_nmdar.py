...
key_ref = 'tether'
in_star_1 = ROOT_PATH + '/pre/ref_nomb_1_clean/ltomos_clst_flt_high_lap/clst_flt_high_lap_ltomos.star'
key_1 = 'pre'
...
in_wspace = ROOT_PATH + '/pst/ampar_vs_nmdar/org/coloc/coloc_tether_vs_pre_ampar_nmdar/coloc_5_60_5_sim_2_global_test_wspace.pkl' # (Insert a path to recover a pickled workspace instead of doing a new computation)

# Output directory
out_dir = ROOT_PATH + '/pst/ampar_vs_nmdar/org/coloc/coloc_tether_vs_pre_ampar_nmdar'
out_stem = 'coloc_5_60_5_sim_2_global_test' # ''uni_sph_4_60_5'
...
ana_rg = np.arange(5, 60, 5) # np.arange(4, 100, 2) # in nm
        ...
        for lt, tkey in enumerate(tomos_hash.keys()):
            ...
            if ana_global:
                hold_arr_1, hold_arr_2 = ref_tomo.simulate_bi_2nd_order(ltomo, p_nsims, ModelCSRV, part_vtp,
                                                                    ...
                                                                    conv_iter=ana_conv_iter, max_iter=ana_max_iter,
                                                                    npr=ana_npr, npr_model=ana_npr_model, out_sep=2,
...
for tkey, ltomo in zip(tomos_aln.iterkeys(), tomos_aln.itervalues()):
    ...
    plt.xticks((1, 2, 3), ('PRE-C', 'AMPAR', 'NMDAR'))
...
for lkey, tlist in zip(lists_exp.iterkeys(), lists_exp.itervalues()):
    ...
    if ana_global:
        if lkey == key_1:
            plt.plot(ana_rg, np.asarray(tlist).mean(axis=0), color='blue', linewidth=2, linestyle='-', label='TETHER$\Rightarrow$PRE-C')
        elif lkey == key_2:

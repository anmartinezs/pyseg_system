...
# Input STAR file
in_star_pre = ROOT_PATH + '/../../nrt/ltomos_k4_gather_premb_mask/k4_gather_premb_mask_ltomos.star'
key_pre = 'PRE'
in_star_pre_a = ROOT_PATH + '/../../../pre/ref_nomb_1_clean/ltomos_pre_premb_ABC/pre_premb_ABC_ltomos.star' # '/../../nrt/ltomos_clst_flt_high/k4_gather_clst_flt_high_ltomos.star'
key_pre_a = 'PRE_A' # 'PST_AP'
in_star_pre_b = ROOT_PATH + '/../../../pre/ref_nomb_1_clean/ltomos_pre_premb_ABC/pre_premb_ABC_ltomos.star' # '/../../nrt/ltomos_clst_flt_high/k4_gather_clst_flt_high_ltomos.star' # '/../../../pre/ref_nomb_1_clean/ltomos_pre_premb_gather_mask/pre_premb_gather_mask_ltomos.star'
key_pre_b = 'PRE_B' # 'PRE'
in_star_pre_c = ROOT_PATH + '/../../../pre/ref_nomb_1_clean/ltomos_pre_premb_ABC/pre_premb_ABC_ltomos.star' # '/../../nrt/ltomos_clst_flt_high/k4_gather_clst_flt_high_ltomos.star' # '/../../../pre/ref_nomb_1_clean/ltomos_clst_flt_high/pre_gather_clst_flt_high_ltomos.star'
key_pre_c = 'PRE_C' # 'PRE_AP'
in_tethers_csv = ROOT_PATH + '/../../../pre/ref_nomb_1_clean/py_scripts/syn_num_tethers.csv'
in_wspace = ROOT_PATH + '/uni_sph_pre_100_min_5_svzero/uni_15_105_5_sim_200_local_ssup_8_wspace.pkl' # (Insert a path to recover a pickled workspace instead of doing a new computation)

# Output directory
out_dir = ROOT_PATH + '/uni_sph_pre_100_min_5_svzero'
...
if in_wspace is None:
    ...
    try:
        ...
        sys.exit(-1)
    ltomos_pkl = star_pre.get_element('_psPickleFile', 0)
    list_pre = unpickle_obj(ltomos_pkl)
    ltomos_pkl = star_pre_a.get_element('_psPickleFile', 0)
    list_pre_a = unpickle_obj(ltomos_pkl)
    ltomos_pkl = star_pre_b.get_element('_psPickleFile', 1)
    list_pre_b = unpickle_obj(ltomos_pkl)
    ltomos_pkl = star_pre_c.get_element('_psPickleFile', 2)
...
for tkey in tomos_den.iterkeys():
    ...
    if np.asarray(tomos_np[tkey].values()).sum() > 0:
        ...
        l_wnum_pre.append(1.)
        l_num_pre_a.append(tomos_np[tkey]['PRE_A'])
        l_wnum_pre_a.append(1.)
        l_num_pre_b.append(tomos_np[tkey]['PRE_B'])
        l_wnum_pre_b.append(1.)
        l_num_pre_c.append(tomos_np[tkey]['PRE_C'])
        l_wnum_pre_c.append(1.)
        if l_num_pre[-1] > p_max_np:
            p_max_np = l_num_pre[-1]
l_ves = np.asarray(l_ves, dtype=np.float).reshape(-1, 1)
l_num_pre = np.asarray(l_num_pre, dtype=np.float).reshape(-1, 1)
l_num_pre_a = np.asarray(l_num_pre_a, dtype=np.float).reshape(-1, 1)
l_num_pre_b = np.asarray(l_num_pre_b, dtype=np.float).reshape(-1, 1)
l_num_pre_c = np.asarray(l_num_pre_c, dtype=np.float).reshape(-1, 1)
l_wnum_pre = np.asarray(l_wnum_pre, dtype=np.float)
l_wnum_pre_a = np.asarray(l_wnum_pre_a, dtype=np.float)
l_wnum_pre_b = np.asarray(l_wnum_pre_b, dtype=np.float)
l_wnum_pre_c = np.asarray(l_wnum_pre_c, dtype=np.float)
...
plt.xticks((1, 2, 4, 5, 7, 8, 10, 11),
           ('PRE', 'PRE*', 'PRE_A', 'PRE_A*', 'PRE_B', 'PRE_B*', 'PRE_C', 'PRE_C*'),
           fontsize=10)
plt.ylim((-1, 1))
plt.tight_layout()
if fig_fmt is None:
...
high_pvals = dict()
for lkey in ('PRE', 'PRE_A', 'PRE_B', 'PRE_C'):
...
for tkey, tomo_exp in zip(tomos_exp.iterkeys(), tomos_exp.itervalues()):
    ...
    for lkey in ('PRE', 'PRE_A', 'PRE_B', 'PRE_C'):
        ...
        if lkey == 'PRE':
            plt.plot(ana_rg, arr_exp, color='k', linewidth=2, linestyle='-', label='PRE')
        elif lkey == 'PRE_A':
            plt.plot(ana_rg, arr_exp, color='blue', linewidth=2, linestyle='-', label='PRE_A')
        if lkey == 'PRE_B':
            plt.plot(ana_rg, arr_exp, color='blue', linewidth=2, linestyle='--', label='PRE_B')
        elif lkey == 'PRE_C':
            plt.plot(ana_rg, arr_exp, color='blue', linewidth=2, linestyle='-.', label='PRE_C')
...
for i, lkey in enumerate(('PRE', 'PRE_A', 'PRE_B', 'PRE_C')):
...
for lkey, tlist in zip(lists_exp.iterkeys(), lists_exp.itervalues()):
    ...
    if lkey == 'PRE':
        plt.plot(ana_rg, tlist, color='k', linewidth=2, linestyle='-', label='PRE')
    elif lkey == 'PRE_A':
        plt.plot(ana_rg, tlist, color='blue', linewidth=2, linestyle='-', label='PRE_A')
    if lkey == 'PRE_B':
        plt.plot(ana_rg, tlist, color='blue', linewidth=2, linestyle='--', label='PRE_B')
    elif lkey == 'PRE_C':
        plt.plot(ana_rg, tlist, color='blue', linewidth=2, linestyle='-.', label='PRE_C')
    plt.xlim(pt_xmin, pt_xmax)
    plt.legend(loc=4)
    plt.tight_layout()
    if fig_fmt is None:
...
for lkey, tlist in zip(lists_exp.iterkeys(), lists_exp.itervalues()):
    ...
    if lkey == 'PRE':
        plt.plot(ana_rg, p_values, color='k', linewidth=2, linestyle='-', label='PRE')
    elif lkey == 'PRE_A':
        plt.plot(ana_rg, p_values, color='blue', linewidth=2, linestyle='-', label='PRE_A')
    if lkey == 'PRE_B':
        plt.plot(ana_rg, p_values, color='blue', linewidth=2, linestyle='--', label='PRE_B')
    elif lkey == 'PRE_C':
        plt.plot(ana_rg, p_values, color='blue', linewidth=2, linestyle='-.', label='PRE_C')
plt.xlim(pt_xmin, pt_xmax)
plt.ylim(0, 1.1)
plt.legend(loc=4)
plt.tight_layout()
...
for lkey, tlist in zip(lists_exp.iterkeys(), lists_exp.itervalues()):
    ...
    if lkey == 'PRE':
        plt.plot(ana_rg, dtlist, color='k', linewidth=2, linestyle='-', label='PRE')
    elif lkey == 'PRE_A':
        plt.plot(ana_rg, dtlist, color='blue', linewidth=2, linestyle='-', label='PRE_A')
    if lkey == 'PRE_B':
        plt.plot(ana_rg, dtlist, color='blue', linewidth=2, linestyle='--', label='PRE_B')
    elif lkey == 'PRE_C':
        plt.plot(ana_rg, dtlist, color='blue', linewidth=2, linestyle='-.', label='PRE_C')
# plt.ylim(-0.7, 1.5)
plt.xlim(pt_xmin, pt_xmax)
plt.legend(loc=1)
plt.tight_layout()
...
for lkey, tlist in zip(lists_exp_ctrl.iterkeys(), lists_exp_ctrl.itervalues()):
    ...
    if lkey == 'PRE':
        ...
        plt.plot(ana_rg, tlist, color='b', linewidth=2, linestyle='-', label='PRE_A')
    if lkey == 'PRE_B':
        plt.plot(ana_rg, tlist, color='b', linewidth=2, linestyle='--', label='PRE_B')
    elif lkey == 'PRE_C':
        plt.plot(ana_rg, tlist, color='b', linewidth=2, linestyle='-.', label='PRE_C')
    plt.xlim(pt_xmin, pt_xmax)
    plt.legend(loc=4)
    plt.tight_layout()
    if fig_fmt is None:
...
for lkey, tlist in zip(lists_exp_ctrl.iterkeys(), lists_exp_ctrl.itervalues()):
    ...
    if lkey == 'PRE':
        ...
        plt.plot(ana_rg, p_values, color='b', linewidth=2, linestyle='-', label='PRE_A')
    if lkey == 'PRE_B':
        plt.plot(ana_rg, p_values, color='b', linewidth=2, linestyle='--', label='PRE_B')
    elif lkey == 'PRE_C':
        plt.plot(ana_rg, p_values, color='b', linewidth=2, linestyle='-.', label='PRE_C')
plt.xlim(pt_xmin, pt_xmax)
plt.ylim(0, 1.1)
plt.legend(loc=4)
plt.tight_layout()

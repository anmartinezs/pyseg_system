...
ROOT_PATH = '/fs/pool/pool-lucic2/antonio/workspace/psd_an/ex/syn/sub/relion/fils/az'

# Input STAR file
in_star = ROOT_PATH + '/ref_nomb_1_clean/ref_1_surfs.star'
...
# Output directory
out_dir = ROOT_PATH + '/ref_nomb_1_clean/ltomos_az_ref_1_gather_premb_cleft' # '/ref_a3/ltomos'
out_stem = 'az_cleft_premb_mask' # 'pre'
...
if sg_pj:
    ...
    if sg_pj_lbl is not None:
        print '\t\t-VOI label for projection: ' + str(sg_pj_lbl)
if sg_voi_mask:
...
for tomo_row in range(star_seg.get_nrows()):
    ...
    if sg_voi_mask:
        ...
        vois[seg_str] = voi
    if sg_pj_lbl is not None:
        tomo = (tomo == sg_pj_lbl).astype(np.float32)
        if (sg_sg is not None) and (sg_sg > 0):
            tomo = sp.ndimage.filters.gaussian_filter(tomo, sg_sg)
        voi = surf.iso_surface(tomo, 0.5, closed=True, normals='outwards')
        if sg_dec is not None:
...
for star_row in range(star.get_nrows()):
    ...
    for part_row in range(star_part.get_nrows()):
        ...
        # Initial rotation
        try:
            ox = star_part.get_element('_rlnOriginX', part_row)
        except KeyError:
            ox = 0
        try:
            oy = star_part.get_element('_rlnOriginY', part_row)
        except KeyError:
            oy = 0
        try:
            oz = star_part.get_element('_rlnOriginZ', part_row)
        except KeyError:
            oz = 0

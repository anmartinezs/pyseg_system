...
if in_wspace_bi is None:
    ...
    for lt, tkey in enumerate(tomos_hash.iterkeys()):
        ...
        print '\t\t\t\t*Processing tomogram (tomo ' + str(lt + 1) + ' of ' + \
              str(len(tomos_hash.keys())) + ') : ' + os.path.split(tkey)[1]
    ...
    for tkey in hold_list_exp_0.iterkeys():
        if ana_global:
    ...
    for tkey in hold_list_exp_1.iterkeys():
        if ana_global:

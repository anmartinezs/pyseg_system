...
# $Id: clusters.py 1461 2017-10-12 10:10:49Z vladan $
"""
...
class Clusters(Groups):
    ...
    def addDistances(self, files, catalog, clusters=None):
        ...
        if files == 'default':
            ...
            for categ, ident_dict in db.files.iteritems():
                for ident, file_name in db.files[categ].iteritems():
    ...
    def removeBoundaries(self, ids):
        ...
        # remove boundary ids
        for categ in ids.keys():

            # continue if current category not this instance
            if categ not in self.keys():
                continue
    ...
    def findDistance(self, items, distance, mode=None, categories=None):
        ...
        for categ in categories:
            ...
            for obs_ind in range(len(self[categ].identifiers)):
                ...
                dist_dict = dict(zip(item_ids[obs_ind], item_dist[obs_ind])) 

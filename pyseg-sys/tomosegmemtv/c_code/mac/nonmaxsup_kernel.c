 ...
 *  nonmaxsup_kernel.c
 ...
 Kernel for manifold detection by non-maximum suppresion criteria, input data must be 
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )
{
	...
	if (num_threads<1) {
		mexErrMsgTxt("nonmaxsup_stub.c: No active cpu detected.");
		return;
	}
	dat64 = -1;
	len64 = sizeof( dat64 );
	...
	if (dat64<1) {
		mexErrMsgTxt("nonmaxsup_stub.c: No Cache L2 detected.");
		return;
	}

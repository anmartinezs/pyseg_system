...
# Input STAR files
in_star = ROOT_PATH + '/data/tutorials/synth_sumb/org/ltomos/test_1/test_1_ltomos.star'
in_wspace = None # ROOT_PATH + '/data/tutorials/synth_sumb/org/uni_1st/test_1_100_60_sim_5_wspace.pkl' # (Insert a path to recover a pickled workspace instead of doing a new computation)

# Output directory
out_dir = ROOT_PATH + '/data/tutorials/synth_sumb/org/uni_1st/'
out_stem = 'test_1_100_60_sim_5'

# Analysis variables
ana_res = 1.048 # 0.684 # nm/voxel
ana_nbins = 100
ana_rmax = 60 # nm
ana_f_npoints = 1000
...
if mp_npr is None:
    ...
    mp_npr = p_nsims
sim_ids = np.array_split(range(p_nsims), np.ceil(float(p_nsims)/mp_npr))
print '\t\t-Number of processors for models simulation: ' + str(mp_npr)
    ...
    for lkey, llist in zip(set_lists_dic.iterkeys(), set_lists_dic.itervalues()):
        ...
        for tkey, ltomo in zip(llist_tomos_dic.iterkeys(), llist_tomos_dic.itervalues()):
            try:
                ...
                tomos_exp_mnnd[tkey], tomos_sim_mnnd[tkey] = dict.fromkeys(lists_dic.keys()), dict.fromkeys(lists_dic.keys())
        ...
        for tkey in tomos_hash.iterkeys():
            ...
            for ids in sim_ids:
                ...
                sim_tomos = gen_tlist(len(ids), ltomo.get_num_particles(), temp_model, ltomo.get_voi(), p_vtp,
                                      mode_emb='center', npr=mp_npr)
                if (len(pids_start) > 0) and (len(pids_end) > 0):
                    sim_tomos2 = gen_tlist(len(ids), ltomo.get_num_particles(), temp_model2, ltomo.get_voi(), p_vtp,
                                           mode_emb='center', npr=mp_npr, in_coords=ltomo.get_particle_coords())
